...
public class CustomPhoneNumberFormattingTextWatcher implements TextWatcher {
    ...
    private OnPhoneChangedListener onPhoneChangedListener;
    ...
    public CustomPhoneNumberFormattingTextWatcher(OnPhoneChangedListener listener) {
        this(Locale.US.getCountry());
        onPhoneChangedListener = listener;
    }
    ...
    public synchronized void afterTextChanged(Editable s) {
        if (stopFormatting) {
            ...
            stopFormatting = !(s.length() == 0 || s.toString().equals(PLUS_SIGN));
            return;
        }
        if (selfChange) {
        ...
        if (formatted != null) {
            ...
            selfChange = true;
            s = s.replace(0, s.length(), formatted, 0, formatted.length());
            if (s.charAt(0) != PLUS_SIGN.charAt(0)) {
                s = s.insert(0, PLUS_SIGN);
            }
