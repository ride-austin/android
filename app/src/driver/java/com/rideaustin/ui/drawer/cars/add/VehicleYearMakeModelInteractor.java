...
public class VehicleYearMakeModelInteractor implements SetupVehicleInteractor {
    ...
    private final AddCarViewModel viewModel;

    public VehicleYearMakeModelInteractor(Context context, VehicleModel.FieldType myType, VehicleManager manager, AddCarViewModel viewModel) {
    ...
    public void onListItemSelected(String value) {
        applyCarValue(viewModel.getCarRegistration(), myType, value);
        manager.applyFilter(myType, value);
    }

    @Override
    public void clear() {
        Timber.d("removeExisting %s", myType);
